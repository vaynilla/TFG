...
(ó enfriamiento) $U(t)$ quedan descritas en términos de energía por unidad de tiempo
...
\begin{equation}
	e^{Kt}\dfrac{d T}{d t}(t) + Ke^{Kt}T(t) = e^{Kt}Q(t),
\end{equation}
\begin{equation}
...
\begin{ejemplo}
	...
	\end{equation}
	donde $M_0$ es una constante, en cualquier caso, la constante de tiempo es justamente $K$, lo que representa el tiempo que tarda la diferencia de temperaturas $T-M_0$ en cambiar de $T-M_0$ a $(T_0 - m_0)/e$. También diremos que $K$ es la \textbf{constante de tiempo para el edificio} (sin calefacción o aire acondicionado). A mayor sea el valor de esta constante, más tiempo tardará en transferirse el calor entre el exterior y el interior del edificio. Un valor típico para la constante de tiempo de un edificio es de $2$ a $4$ horas, pero puede ser menor si las ventanas están abiertas o existe algún ventilador. También puede ser mayor si el edificio está aislado.
...
\begin{ejemplo}
	...
	\end{equation}
\end{ejemplo}
\section{Modelo con 3 habitaciones}
...
\begin{figure}[h!]
	\centering
	\includegraphics[width=0.7\textwidth]{edificio_3habs}
...
\begin{itemize}
	...
	\item Constante de transferencia entre cada habitación $K = 3$
	\item Constante de transferencia entre una habitación y el exterior $K_{xF} = 5$
...
\end{equation}
Resolvemos el sistema con $Maxima$ y obtenemos la solución gráficamente en la \autoref{fig:graf_sol3}.
\begin{figure}[h!]
	\centering
	\includegraphics[width=\textwidth]{graf_sol3}
