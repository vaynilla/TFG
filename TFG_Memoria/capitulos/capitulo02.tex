...
El método de iteración variacional presenta una funcional corrección para la ecuación~\eqref{eq:miv1} en la forma:
\begin{equation}\label{eq:miv2}
	u_{n+1}(x) = u_n(x) + \int_{0}^{x} \lambda(\xi)(Lu_n(\xi) + Nu'_n(\xi) - g(\xi))d\xi,
\end{equation}
...
\begin{equation}
	\dfrac{\delta u_{n+1}}{\delta u_n} = 1 + \dfrac{\delta}{\delta u_n}(\int_{0}^{x} \lambda(\xi)(Lu_n(\xi)+Nu'_n(\xi)-g(\xi))d\xi),
\end{equation}
o equivalentemente
\begin{equation}
