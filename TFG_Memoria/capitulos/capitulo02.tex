...
\begin{observacion}
	Nótese que en este método, $\lambda$ puede ser una constante o una función, y $û_n$ es un valor restringido, por tanto se comporta como una constante, luego $\delta û_n = 0$, donde $\delta$ es la derivada variacional.
\end{observacion}
...
\end{enumerate}
Tomando la variación de~\eqref{eq:miv2} con respecto a la variable independiente $û_n$, encontramos
\begin{equation}
...
\end{equation}
Es decir, la correción funcional nos dará varias aproximaciones, y por tanto, la solución exacta se obtiene como el límite de todas estas sucesivas aproximaciones.
...
\begin{teorema}
	Si $f(x)$, dada en~\eqref{eq:aprox1}, es una función continua en el intervalo $0 \leqslant x \leqslant a$, y el núcleo $K(x,t)$ también es continuo en el triángulo $0 \leqslant x \leqslant a$, $0 \leqslant t \leqslant x$, la secuencia de aproximaciones sucesivas $u_n(x), n \geqslant 0$ converge a la solución $u(x)$ de la ecuación integral bajo estudio.
\end{teorema}
\begin{observacion}
	...
	\end{equation}
\end{observacion}
La diferencia entre ambas fórmulas se basa principalmente en:
\begin{enumerate}
...
\end{equation}
Ahora veremos un ejemplo para ilustrar este método.
\begin{ejemplo}
...
\begin{definicion}
	Una función real $u(x)$ se llama analítica si existe derivada de cualquier orden de forma que la serie de Taylor en cualquier punto b de su dominio
	\begin{equation}
...
\begin{equation}
	u(x) = f(x) + \lambda \int_0^x K(x,t)u(t)dt,
\end{equation}
es analítica, y por tanto posee una forma en serie de Taylor, donde los coeficientes $a_n$ se determinará de forma recursiva. Sustituimos~\eqref{eq:serie1} en ambos lados y obtenemos
...
Ahora la solución en series se sigue inmediatamente sustituyendo estos coeficientes. La solución exacta se obtendrá si existe, y si no, entonces la serie obtenida se puede utilizar con objetivos numéricos. En este caso, a más términos evaluemos, mayor precisión obtendremos.
