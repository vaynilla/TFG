...
Para apreciar mejor en qué estan basados cada uno de los métodos, vamos a distinguir entre los métodos que utilizan series, los métodos iterativos, y otros métodos especiales que no se parecen tanto a los anteriores.
...
\begin{teorema}
	Si $f(x)$, dada en~\eqref{eq:aprox1}, es una función continua para $x \in [0,a]$, y el núcleo $K(x,t)$ también es continuo para $x \in [0,a]$, $t \in [0,x]$, la sucesión de aproximaciones sucesivas $u_n(x), n \geqslant 0$ converge a la solución $u(x)$ de la ecuación integral bajo estudio.
\end{teorema}
\begin{observacion}
